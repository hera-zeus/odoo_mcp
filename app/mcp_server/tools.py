...
async def _get_forecast(args: Dict, odoo_session_id: str) -> str:
    ...
    try:
        ...
        is_prophet  = model_info.get("algo") == "Prophet"

        # Tableau prévision : valeur centrale + bornes si Prophet
        if is_prophet and "forecast_lower" in result and "forecast_upper" in result:
            forecast_dict = {
                str(d.date()): {
